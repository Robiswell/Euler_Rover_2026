    ...
    try:
        ...
        while is_running.value:
            ...
            if temp_spike_counter > 15 or volt_dip_counter > 150:  # 150 ticks = 3s debounce — extended for cold-weather sag during heavy terrain
                ...
                # Fix H1: zero servos immediately before disk I/O
                for _sid in ALL_SERVOS:
                    packet_handler.write2ByteTxRx(port_handler, _sid, ADDR_GOAL_SPEED, 0)
                is_running.value = False
                break
            ...
            cur_roll_mode = shared_roll_mode.value
            ...
            for sid in ALL_SERVOS:
                ...
                if cur_roll_mode:
                    ...
                    if sid in LEFT_SERVOS and not cur_roll_mode:
                        raw_speed = -raw_speed
...
if __name__ == "__main__":
    ...
    MISSION_TIMEOUT_S = 1000  # extended to 1000s (~16.7 min) — unknown course length, battery supports 37-60 min
    ...
    class NavStateMachine:
        ...
        def update(self, frame, imu, front_class, left_class, right_class,
            ...
            if self.stall_count_30s > 0 and (now - self.last_stall_time) > 60:
                self.stall_count_30s = 0
                self.stall_speed_mult = 1.0
                self.terrain_gait = 2  # Fix H4: restore default quad (wave override from 3-stall was permanent)
